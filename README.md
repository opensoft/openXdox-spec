...
| [docs/gate-loop-view-contract.md](docs/gate-loop-view-contract.md) | `Status: draft` — the VIEW CONTRACT a contributed `ViewBinding` may assume about openDox's shell (openDox-spec `docs/front-end-package-boundary.md` § 5.1, the counterpart that note names and does not author): the twelve mount regions measured at the S3 head and what their hosts guarantee (plus a Q8-ruled thirteenth, `page-overlay`, not yet landed in code), the fourteen context-object fields marked STABLE or UNSETTLED, the `/capabilities` payload and its absent-vs-malformed answers, the refusals a contributing column meets on the assembly-and-resolve path (quoted, with the declaration-shape and extension-conformance families named and scoped out) — and the twelve questions S5 needs before the gate loop can be contributed, eleven RULED by amendment #1 (2026-09-12), Q6 left open |
